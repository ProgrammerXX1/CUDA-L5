...
static std::string read_file_prefix(const fs::path& p, size_t max_bytes) {
  ...
  // read a bit more to avoid cutting mid-UTF8 sequence
  const size_t extra = 16;
  const size_t want = max_bytes + extra;
...
ExtractedText extract_text_from_file(const fs::path& p, bool assume_normalized, size_t max_bytes) {
  ...
  // If we have a cap, take UTF-8 safe prefix first (for “true UTF-8 but cut mid char” case).
  if (max_bytes > 0 && raw.size() > max_bytes) {
    const size_t cut = utf8_safe_prefix_len(raw, max_bytes);
    // keep `raw` full for validation attempt below; we will cut appropriately later
    (void)cut;
  }
